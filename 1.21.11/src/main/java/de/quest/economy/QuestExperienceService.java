...
public final class QuestExperienceService {
    ...
    public static double levelBars(int rewardUnits, int projectBonusUnits, RewardType rewardType) {
        ...
        return roundToQuarterBar(baseReward + projectBonusBars(projectBonusUnits));
    }

    public static double projectBonusBars(int projectBonusUnits) {
        if (projectBonusUnits <= 0) return 0.0d;
        return roundToQuarterBar(projectBonusUnits * PROJECT_BONUS_BARS_PER_UNIT);
    }

    public static double grant(ServerPlayerEntity player, int rewardUnits, int projectBonusUnits, RewardType rewardType) {
        double bars = levelBars(rewardUnits, projectBonusUnits, rewardType);
        if (player == null || bars <= 0.0d) return bars;
        int wholeLevels = (int) Math.floor(bars);
        if (wholeLevels > 0) player.addExperienceLevels(wholeLevels);
        double partialBar = bars - wholeLevels;
        if (partialBar > 0.0d) {
            player.addExperience(Math.max(1, (int) Math.round(player.getNextLevelExperience() * partialBar)));
        }
        return bars;
    }

    public static Text rewardLine(int rewardUnits, RewardType rewardType) {
        return rewardLine(rewardUnits, 0, rewardType);
    }

    public static Text rewardLine(int rewardUnits, int projectBonusUnits, RewardType rewardType) {
        return Text.translatable("screen.village-quest.questmaster.reward.experience",
                formatBars(levelBars(rewardUnits, projectBonusUnits, rewardType))).formatted(Formatting.GREEN);
    }

    public static Text projectBonusAmount(int projectBonusUnits) {
        return Text.translatable("text.village-quest.experience.level_amount",
                formatBars(projectBonusBars(projectBonusUnits))).formatted(Formatting.GREEN);
    }
