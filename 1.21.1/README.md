# Village Quest for Minecraft 1.21.1
...
Current release candidate for this folder: `2.1.1 - Homesteads & Wayfinding`, downported from the Yarn `1.21.11` line with target-appropriate 1.21.1 APIs.

This is an explicit Yarn downport of the `2.1.1` feature set. Future content development continues on Minecraft `26.2`. Treat publication and support-policy updates as a separate maintainer decision.
...
- Java `21` (Loom fails if `JAVA_HOME` still points at an older JDK)

## Build tip (PowerShell)

If `java -version` shows 21 but Gradle still complains about Java 17, your shell `JAVA_HOME` is wrong. Point it at JDK 21 for this session:
...
2. Put `village-quest-<version>-mc1.21.1.jar` into your `mods` folder.
