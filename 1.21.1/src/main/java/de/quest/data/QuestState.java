...
public final class QuestState extends PersistentState {
    ...
    public static final PersistentState.Type<QuestState> TYPE =
            new PersistentState.Type<>(QuestState::new, QuestState::createFromNbt, DataFixTypes.LEVEL);
    ...
    private static QuestState createFromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
    ...
    public NbtCompound writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registryLookup) {
        nbt.put("questManager", writeDailyQuestData());
        nbt.put("questPartyManager", questPartyState);
        return nbt;
    }
    ...
    private void readFromNbt(NbtCompound root) {
        ...
        NbtCompound daily = NbtCompat.compound(root, "questManager");
        readDailyQuestData(daily);
        questPartyState = NbtCompat.compound(root, "questPartyManager");
    }
    ...
    private void readDailyQuestData(NbtCompound root) {
        this.pilgrimNaturalSpawnCooldownUntil = Math.max(0L, root.getLong("pilgrimNaturalSpawnCooldownUntil"));
    ...
    private static void readUuidLongMap(NbtCompound root, String key, BiConsumer<UUID, Long> consumer) {
        NbtList list = NbtCompat.compoundList(root, key);
        for (int i = 0; i < list.size(); i++) {
            NbtCompound item = NbtCompat.compoundAt(list, i);
            UUID id = parseUuid(item.getString("id"));
            if (id != null) {
                consumer.accept(id, item.getLong("v"));
            }
        }
    }
    ...
    private static void readUuidIntMap(NbtCompound root, String key, BiConsumer<UUID, Integer> consumer) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            UUID id = parseUuid(item.getString("id"));
            if (id != null) {
                consumer.accept(id, item.getInt("v"));
            }
        }
    }
    ...
    private static void readUuidBoolMap(NbtCompound root, String key, BiConsumer<UUID, Boolean> consumer) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            UUID id = parseUuid(item.getString("id"));
            if (id != null) {
                consumer.accept(id, item.getBoolean("v"));
            }
        }
    }
    ...
    private static void readUuidNamedIntMap(NbtCompound root, String key, NamedIntConsumer consumer) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (id != null && !stateKey.isEmpty()) {
                consumer.accept(id, stateKey, item.getInt("v"));
            }
        }
    }
    ...
    private void readSpecialShardQuestData(NbtCompound root) {
        NbtList list = NbtCompat.compoundList(root, "specialShardQuest");
        for (int i = 0; i < list.size(); i++) {
            NbtCompound item = NbtCompat.compoundAt(list, i);
            UUID id = parseUuid(item.getString("id"));
            if (id == null) {
            ...
            if (item.getBoolean("pendingOffer") && data.getPendingSpecialOfferKind() == null) {
                data.setPendingSpecialOfferKind(SpecialQuestKind.SHARD_RELIC);
            }
            try {
                data.setShardRelicQuestStage(ShardRelicQuestStage.valueOf(NbtCompat.getString(item, "stage", ShardRelicQuestStage.NONE.name())));
            } catch (IllegalArgumentException ex) {
                data.setShardRelicQuestStage(ShardRelicQuestStage.NONE);
            }
            data.setShardRelicAmethystProgress(item.getInt("amethyst"));
            data.setShardRelicPotionProgress(item.getInt("potions"));
            data.setShardRelicEnchantProgress(item.getInt("enchants"));
            data.setShardRelicEnderPearlProgress(item.getInt("enderPearls"));
            data.setShardRelicBlazeRodProgress(item.getInt("blazeRods"));
            data.setShardRelicEnchantBaseline(item.getInt("enchantBase"));
            data.setShardRelicEnderPearlBaseline(item.getInt("enderPearlBase"));
            data.setShardRelicBlazeRodBaseline(item.getInt("blazeRodBase"));
            data.setShardRelicChestX(item.getInt("chestX"));
            data.setShardRelicChestY(NbtCompat.getInt(item, "chestY", Integer.MIN_VALUE));
            data.setShardRelicChestZ(item.getInt("chestZ"));
        }
    }
    ...
    private void readSpecialMerchantSealQuestData(NbtCompound root) {
        NbtList list = NbtCompat.compoundList(root, "specialMerchantSealQuest");
        for (int i = 0; i < list.size(); i++) {
            NbtCompound item = NbtCompat.compoundAt(list, i);
            UUID id = parseUuid(item.getString("id"));
            if (id == null) {
            ...
            PlayerQuestData data = getPlayerData(id);
            try {
                data.setMerchantSealQuestStage(RelicQuestStage.valueOf(NbtCompat.getString(item, "stage", RelicQuestStage.NONE.name())));
            } catch (IllegalArgumentException ex) {
                data.setMerchantSealQuestStage(RelicQuestStage.NONE);
            }
            data.setMerchantSealTradeProgress(item.getInt("trades"));
            data.setMerchantSealEmeraldProgress(item.getInt("emeralds"));
            data.setMerchantSealVillagerPurchaseProgress(item.getInt("villagerPurchases"));
            data.setMerchantSealPilgrimPurchaseProgress(item.getInt("pilgrimPurchases"));
            data.setMerchantSealLastUseDay(NbtCompat.getLong(item, "lastUseDay", PlayerQuestData.UNSET_DAY));
        }
    }
    ...
    private void readSpecialShepherdFluteQuestData(NbtCompound root) {
        NbtList list = NbtCompat.compoundList(root, "specialShepherdFluteQuest");
        for (int i = 0; i < list.size(); i++) {
            NbtCompound item = NbtCompat.compoundAt(list, i);
            UUID id = parseUuid(item.getString("id"));
            if (id == null) {
            ...
            PlayerQuestData data = getPlayerData(id);
            try {
                data.setShepherdFluteQuestStage(RelicQuestStage.valueOf(NbtCompat.getString(item, "stage", RelicQuestStage.NONE.name())));
            } catch (IllegalArgumentException ex) {
                data.setShepherdFluteQuestStage(RelicQuestStage.NONE);
            }
            data.setShepherdFluteBreedProgress(item.getInt("breed"));
            data.setShepherdFluteShearProgress(item.getInt("shear"));
            data.setShepherdFluteWoolProgress(item.getInt("wool"));
            data.setShepherdFluteWoolBaseline(item.getInt("woolBaseline"));
        }
    }
    ...
    private void readSpecialApiaristSmokerQuestData(NbtCompound root) {
        NbtList list = NbtCompat.compoundList(root, "specialApiaristSmokerQuest");
        for (int i = 0; i < list.size(); i++) {
            NbtCompound item = NbtCompat.compoundAt(list, i);
            UUID id = parseUuid(item.getString("id"));
            if (id == null) {
            ...
            PlayerQuestData data = getPlayerData(id);
            try {
                data.setApiaristSmokerQuestStage(RelicQuestStage.valueOf(NbtCompat.getString(item, "stage", RelicQuestStage.NONE.name())));
            } catch (IllegalArgumentException ex) {
                data.setApiaristSmokerQuestStage(RelicQuestStage.NONE);
            }
            data.setApiaristSmokerHoneyProgress(item.getInt("honey"));
            data.setApiaristSmokerCombProgress(item.getInt("comb"));
            data.setApiaristSmokerBeeBreedProgress(item.getInt("beeBreed"));
            data.setApiaristSmokerHoneyBlockProgress(item.getInt("honeyBlock"));
            data.setApiaristSmokerHoneyBlockBaseline(item.getInt("honeyBlockBaseline"));
            data.setApiaristSmokerLastUseDay(NbtCompat.getLong(item, "lastUseDay", PlayerQuestData.UNSET_DAY));
            data.setApiaristSmokerUsesToday(item.getInt("usesToday"));
        }
    }
    ...
    private void readSpecialSurveyorCompassQuestData(NbtCompound root) {
        NbtList list = NbtCompat.compoundList(root, "specialSurveyorCompassQuest");
        for (int i = 0; i < list.size(); i++) {
            NbtCompound item = NbtCompat.compoundAt(list, i);
            UUID id = parseUuid(item.getString("id"));
            if (id == null) {
            ...
            PlayerQuestData data = getPlayerData(id);
            try {
                data.setSurveyorCompassQuestStage(RelicQuestStage.valueOf(NbtCompat.getString(item, "stage", RelicQuestStage.NONE.name())));
            } catch (IllegalArgumentException ex) {
                data.setSurveyorCompassQuestStage(RelicQuestStage.NONE);
            }
            data.setSurveyorCompassRedstoneProgress(item.getInt("redstone"));
            data.setSurveyorCompassLapisProgress(item.getInt("lapis"));
            data.setSurveyorCompassCraftedProgress(item.getInt("crafted"));
            data.setSurveyorCompassPickaxeReadyProgress(item.getInt("pickaxeReady"));
            data.setSurveyorCompassPickaxeBaseline(item.getInt("pickaxeBaseline"));
            data.setSurveyorCompassModeIndex(item.getInt("modeIndex"));
            data.setSurveyorCompassHomeCooldownUntil(item.getLong("homeCooldownUntil"));
            data.setSurveyorCompassHomeConfirmUntil(item.getLong("homeConfirmUntil"));
        }
    }
