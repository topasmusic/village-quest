...
public final class QuestMasterEntity extends PathAwareEntity {
    ...
    public void tickMovement() {
        ...
        if (hasCustomer()) {
            ...
            this.limbAnimator.updateLimbs(0.05f, 0.15f);
        }
    }
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if (this.getHealth() <= this.getMaxHealth() * DEFENSE_TRIGGER_HEALTH_FRACTION) {
            enterDefense((ServerWorld) this.getWorld(), attacker);
        } else {
    ...
    public void readCustomDataFromNbt(NbtCompound data) {
        ...
        this.customerHoldTicks = Math.max(0, NbtCompat.getInt(data, "CustomerHoldTicks", 0));
        this.customerUuid = null;
        this.arrivalTrackTicks = Math.max(0, NbtCompat.getInt(data, "ArrivalTrackTicks", 0));
        String encodedArrivalTarget = NbtCompat.getString(data, "ArrivalTarget", "");
    ...
    private void attemptDefenseAttack(ServerPlayerEntity target) {
        ...
        boolean hit = this.tryAttack(target);
        if (!hit) {
            float damage = (float) this.getAttributeValue(EntityAttributes.GENERIC_ATTACK_DAMAGE);
            hit = target.damage(damageSource, damage);
