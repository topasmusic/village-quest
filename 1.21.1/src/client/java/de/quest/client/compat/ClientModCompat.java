    ...
    public static void bootstrap() {
        ...
        if (BENDABLE_CUBOIDS_LOADED) {
            VillageQuest.LOGGER.warn(
                    "Detected Bendable Cuboids on 1.21.1. Village Quest keeps the standard held-item renderer; report crashes if this combo misbehaves."
            );
        }
        if (MTG_CARD_LOADED) {
            VillageQuest.LOGGER.warn(
                    "Detected MTGCard on 1.21.1. Village Quest is disabling the old inventory journal overlay to avoid known inventory-screen crashes and is using a fallback inventory Journal button instead."
            );
        }
    }
