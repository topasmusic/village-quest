...
public final class TradeRouteMapScreen extends ResponsiveScreen {
    ...
    public TradeRouteMapScreen(Payloads.TradeRouteMapPayload data) {
        super(Text.translatable("screen.village-quest.trade_route.title"));
        this.data = data;
    ...
    public boolean mouseClicked(double mouseXd, double mouseYd, int button) {
        ...
        return switch (viewMode) {
            case MAP -> {
                ...
                yield super.mouseClicked(mouseXd, mouseYd, button);
            }
            case ROUTES -> handleRoutesClick(mouseX, mouseY, left, top) || super.mouseClicked(mouseXd, mouseYd, button);
            case GUIDE -> super.mouseClicked(mouseXd, mouseYd, button);
        };
    }

    @Override
    public boolean mouseDragged(double mouseXd, double mouseYd, int button, double dragX, double dragY) {
        if (mapDragging && button == 0 && viewMode == ViewMode.MAP) {
    ...
    private void drawTabs(DrawContext graphics, int left, int top, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            if (hovered) {
                graphics.drawTooltip(textRenderer, Text.translatable(labels[i]), mouseX, mouseY);
            }
        }
    }
    ...
    private void drawHeader(DrawContext graphics, int left, int top) {
        ...
        graphics.drawText(textRenderer, value, left + (WINDOW_WIDTH - textRenderer.getWidth(value)) / 2,
                top + 14, INK, false);
    }

    private void drawMapView(DrawContext graphics, int left, int top, int mouseX, int mouseY) {
    ...
    private void drawRoutesOnMap(DrawContext graphics, Bounds bounds, int left, int top,
        ...
        for (Payloads.TradeRouteLineData route : sortedRoutes()) {
            ...
            if (route.routeIndex() == selectedRoute) {
                for (Payloads.TradeRoutePointData waypoint : route.waypoints()) {
                    ...
                    if (Math.abs(mouseX - point.x) <= 7 && Math.abs(mouseY - point.y) <= 7) {
                        ...
                        tooltip.add(Text.literal("[" + waypoint.worldX() + ", " + waypoint.worldZ() + "]"));
            ...
            if (caravan != null && config.showCaravanMarkers()) {
                ...
                if (Math.abs(mouseX - point.x) <= 11 && Math.abs(mouseY - point.y) <= 11) {
                    ...
                    tooltip.add(Text.translatable("screen.village-quest.trade_route.caravan_coordinates",
                            (int) Math.round(world.x), (int) Math.round(world.z)));
                    tooltip.add(caravan.boarding()
                            ? Text.translatable("screen.village-quest.trade_route.ferry_boarding")
                            : caravan.ferry()
                            ? Text.translatable("screen.village-quest.trade_route.ferry_crossing",
                            formatEta(caravan.ferrySecondsRemaining()))
                            : Text.translatable(caravan.materialized()
                    ...
                    if (!route.eventLabel().getString().isEmpty()) {
                        tooltip.add(Text.translatable("screen.village-quest.trade_route.event_tooltip",
                                route.eventLabel()));
                    }
                    graphics.drawTooltip(textRenderer, tooltip, mouseX, mouseY);
                }
        ...
        if (config.showVillageMarkers()) {
            for (Payloads.TradeRouteNodeData node : data.nodes()) {
                ...
                if (hovered) {
                    ...
                    tooltip.add(Text.literal("X " + node.worldX() + "  Z " + node.worldZ()));
                    graphics.drawTooltip(textRenderer, tooltip, mouseX, mouseY);
                } else if (zoomLevel >= 2) {
                    String label = VillageUiTheme.ellipsize(textRenderer, node.name().getString(), 80);
                    graphics.drawText(textRenderer, label, point.x - textRenderer.getWidth(label) / 2,
    ...
    private void drawPlayerMarker(DrawContext graphics, Bounds bounds, int left, int top,
        ...
        if (Math.abs(mouseX - player.x) <= 11 && Math.abs(mouseY - player.y) <= 11) {
            graphics.drawTooltip(textRenderer,
                    Text.translatable("screen.village-quest.trade_route.player_coordinates",
                            client.player.getBlockX(), client.player.getBlockZ()), mouseX, mouseY);
        }
    }

    private void drawMapControls(DrawContext graphics, int left, int top, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            VillageUiTheme.drawButton(graphics, textRenderer, x, y, CONTROL_SIZE, CONTROL_SIZE,
        ...
        int zoomWidth = textRenderer.getWidth(zoom) + 6;
        ...
        graphics.drawText(textRenderer, zoom, left + ZOOM_X + CONTROL_SIZE - zoomWidth + 3,
                zoomY + 2, MUTED, false);
    }

    private void drawMapFooter(DrawContext graphics, int left, int top, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            VillageUiTheme.drawButton(graphics, textRenderer, x, y, CHIP_WIDTH, CHIP_HEIGHT,
        ...
        String summary = selected == null
                ? Text.translatable("screen.village-quest.trade_route.empty").getString()
                : selected.name().getString() + " · " + selected.statusLabel().getString();
        summary = VillageUiTheme.ellipsize(textRenderer, summary, 145);
        VillageUiTheme.drawStringScaled(graphics, textRenderer, summary,
        ...
        VillageUiTheme.drawButton(graphics, textRenderer, manageX, top + FOOTER_Y, 72, CHIP_HEIGHT,
                Text.translatable("screen.village-quest.trade_route.manage").getString(),
                true, hovered, false);
    }

    private void drawRoutesView(DrawContext graphics, int left, int top, int mouseX, int mouseY) {
        List<Payloads.TradeRouteLineData> routes = sortedRoutes();
        ...
        for (int i = 0; i < routes.size(); i++) {
            ...
            String status = route.surveying()
                    ? Text.translatable("screen.village-quest.trade_route.surveying").getString()
                    : route.statusLabel().getString();
            VillageUiTheme.drawStringScaled(graphics, textRenderer, compact(status, 105, 0.72f),
                    listX + 14, y + 18, routeColor(route.status()), 0.72f);
            String quality = route.roadQuality() + "%";
            VillageUiTheme.drawStringScaled(graphics, textRenderer, quality,
                    listX + rowWidth - textRenderer.getWidth(quality) * 0.75f - 8,
                    y + 7, MUTED, 0.75f);
            ...
            if (operationHovered) {
                ...
                graphics.drawTooltip(textRenderer, routeTooltip(route), mouseX, mouseY);
            }
        }
        if (routes.isEmpty()) {
            drawWrapped(graphics,
                    Text.translatable("screen.village-quest.trade_route.register_hint").getString(),
        ...
        if (selected != null) {
            ...
            String[] lines = {
                    Text.translatable("screen.village-quest.trade_route.route_stats_short",
                            selected.roadQuality(), selected.waypoints().size()).getString(),
                    Text.translatable("screen.village-quest.trade_route.earnings_short",
                            selected.lifetimeEarnings()).getString(),
                    Text.translatable("screen.village-quest.trade_route.specialization",
        ...
        String summary = VillageUiTheme.ellipsize(textRenderer, data.summary().getString(), VIEW_WIDTH - 8);
        float summaryX = left + VIEW_X + VIEW_WIDTH - 4 - textRenderer.getWidth(summary) * 0.75f;
        VillageUiTheme.drawStringScaled(graphics, textRenderer, summary,
                summaryX, top + FOOTER_Y + 18, MUTED, 0.75f);
    }

    private void drawRouteActions(DrawContext graphics, Payloads.TradeRouteLineData route,
                                  int x, int y, int width, int mouseX, int mouseY) {
        int gap = 4;
        int buttonWidth = (width - gap) / 2;
        drawActionButton(graphics, x, y, buttonWidth,
                Text.translatable(route.paused()
        ...
        drawActionButton(graphics, x + buttonWidth + gap, y, buttonWidth,
                Text.translatable(route.surveying()
        ...
        drawActionButton(graphics, x + buttonWidth + gap, y + 22, buttonWidth,
                Text.translatable(confirming
                        ? "screen.village-quest.trade_route.remove_confirm"
                        : "screen.village-quest.trade_route.remove").getString(),
                true, mouseX, mouseY);
    }

    private void drawActionButton(DrawContext graphics, int x, int y, int width, String label,
                                  boolean enabled, int mouseX, int mouseY) {
        VillageUiTheme.drawButton(graphics, textRenderer, x, y, width, 18, label, enabled,
                enabled && within(mouseX, mouseY, x, y, width, 18), false);
    }

    private void drawGuideView(DrawContext graphics, int left, int top) {
        int x = left + VIEW_X + 10;
        int y = top + VIEW_Y + 8;
        int cardWidth = VIEW_WIDTH - 20;
        VillageUiTheme.drawCard(graphics, x, y, cardWidth, 66, false, true);
        VillageUiTheme.drawStringScaled(graphics, textRenderer,
                Text.translatable("screen.village-quest.trade_route.guide_navigation").getString(),
                x + 24, y + 8, INK, 0.86f);
        VillageUiTheme.drawWrappedScaled(graphics, textRenderer,
                Text.translatable("screen.village-quest.trade_route.guide_navigation.body").getString(),
                x + 24, y + 23, cardWidth - 48, BODY, 0.72f, 4);
        y += 70;
        VillageUiTheme.drawCard(graphics, x, y, cardWidth, 66, false, false);
        VillageUiTheme.drawStringScaled(graphics, textRenderer,
                Text.translatable("screen.village-quest.trade_route.guide_routes").getString(),
                x + 24, y + 8, INK, 0.86f);
        VillageUiTheme.drawWrappedScaled(graphics, textRenderer,
                Text.translatable("screen.village-quest.trade_route.survey_tooltip").getString(),
    ...
    private List<Text> routeTooltip(Payloads.TradeRouteLineData route) {
        List<Text> tooltip = new ArrayList<>();
        addWrappedTooltip(tooltip, route.name());
        addWrappedTooltip(tooltip, Text.translatable(route.paused()
        ...
        addWrappedTooltip(tooltip, Text.translatable(
        ...
        addWrappedTooltip(tooltip, Text.translatable("screen.village-quest.trade_route.specialization",
                route.specializationLabel()));
        addWrappedTooltip(tooltip, Text.translatable("screen.village-quest.trade_route.upgrades",
    ...
    private void addWrappedTooltip(List<Text> tooltip, Text line) {
        ...
        for (String word : text.split("\\s+")) {
            ...
            if (!current.isEmpty() && textRenderer.getWidth(candidate) > 240) {
                tooltip.add(Text.literal(current.toString()));
                current.setLength(0);
                current.append(word);
            } else {
                current.setLength(0);
    ...
    private void openRouteRename(Payloads.TradeRouteLineData route) {
        ...
        notifyClosed();
        client.setScreen(new net.minecraft.client.gui.screen.ChatScreen("/vq routes rename " + (route.routeIndex() + 1) + " "));
    }
    ...
    private void drawLine(DrawContext graphics, int x0, int y0, int x1, int y1, int color, int thickness) {
    ...
    private void drawDashedLine(DrawContext graphics, int x0, int y0, int x1, int y1,
    ...
    private void drawWrapped(DrawContext graphics, String text, int x, int y,
                             int maxWidth, int color, int maxLines) {
        StringBuilder line = new StringBuilder();
        int count = 0;
        for (String word : text.split("\\s+")) {
            String next = line.isEmpty() ? word : line + " " + word;
            if (!line.isEmpty() && textRenderer.getWidth(next) > maxWidth) {
                graphics.drawText(textRenderer, line.toString(), x, y, color, false);
        ...
        if (!line.isEmpty() && count < maxLines) {
            graphics.drawText(textRenderer, VillageUiTheme.ellipsize(textRenderer, line.toString(), maxWidth),
                    x, y, color, false);
        }
    }
