...
- Village Quest 2.4.1 brings the complete Guild Comes to Town adventure to Minecraft 26.3, including Village Contacts, five local stories, Cross-Village Commissions, shared Guild Projects, and The Bells of Concord.
- Guild Notice Posts now guide your first village favor, introductions, local stories, and Shared Table choices in a dedicated village journey interface. Connected villages keep their request page as a second board tab.

### Bug fixes and improvements

- Updated Village Quest for Minecraft 26.3 and its current Fabric and input APIs while preserving existing progression and save data.
- Quest Tracker and Journal welcome hints now describe the configurable tracker position correctly; the default is top-left.
...
## 2.4.1-unreleased.1 (Minecraft 26.3 parity candidate)

- Aligned the 26.3 candidate's Village Quest version with the requested 2.4.1 release pair. Its guided Guild Notice Post journey and preview commands are being matched on Minecraft 26.2 before release.

## 2.4.0-unreleased.5 (Minecraft 26.3 Notice Board interaction polish)

- The Guild Notice Post consumes its opening click without a hand swing, so closing its UI no longer reveals an unnecessary swing animation.

## 2.4.0-unreleased.4 (Minecraft 26.3 Notice Board preview fix)

- Registered the new `noticejourney` admin previews under `/vq admin uitest` as documented. In `.3` they were inadvertently siblings of `uitest`, so the documented commands were rejected. The UI and gameplay behavior are unchanged.
- Added a Brigadier tree regression test for all preview states and the existing Notice Board preview branch.

## 2.4.0-unreleased.3 (Minecraft 26.3 Notice Board UX candidate)

- The Guild Notice Post now opens a village journey page for the first favor, three-villager welcome, and local stories. Story acceptance, Shared Table decisions, progress, and final bundle delivery are available through the board UI; registered villages also offer the existing request page as a second tab. Gameplay requirements and saved progression are unchanged.
- Added read-only `/vq admin uitest noticejourney` previews for welcome, greeting, story stages, all five local story layouts, and a connected board with its request tab. The preview does not change quest or world state.
- Reused the existing painted board, card, and Story icon art with a warmer story layout and localized the new controls in English, German, and Spanish. This is a focused 26.3 UX change; the 26.2 maintenance line has not been changed.

## 2.4.0-unreleased.2 (Minecraft 26.3 candidate)
...
## 2.4.0-unreleased.1 (Minecraft 26.3 port candidate)

- Ported the published 2.4.0 source from Minecraft 26.2 to Minecraft 26.3 with Fabric Loader 0.19.5, Loom 1.17.21, and Fabric API 0.161.0+26.3; Java 25 and Gradle 9.6.0 remain unchanged.
- Adapted SDL-backed key and mouse input, block piston reactions, server-only reward drops, NPC swings, structure template and structure-start APIs, recipe lookup and unlocks, client rendering types and rotations, and the brewed-potion Mixin descriptor.
- Preserved the Stable 2.4.0 gameplay rules, resource keys, and save-facing IDs. This candidate is unreleased pending runtime and existing-world port audit.
