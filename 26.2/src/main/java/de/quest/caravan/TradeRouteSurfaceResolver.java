...
final class TradeRouteSurfaceResolver {
    ...
    static BlockPos findNearbyRoadSurfaceNearY(ServerLevel world, BlockPos center, int radius,
        ...
        for (int dx = -radius; dx <= radius; dx += 2) {
            for (int dz = -radius; dz <= radius; dz += 2) {
                ...
                if (surface == null
                        || !isStableCaravanSurfaceNearY(world, surface)
                        || !isRoadBlock(world.getBlockState(surface.below()))) {
                    continue;
                }
                double distance = surface.distSqr(center);
    ...
    static BlockPos safeSurfaceNearY(ServerLevel world, int x, int y, int z, int verticalTolerance) {
        if (world == null) {
            return null;
        }
        BlockPos chunkProbe = new BlockPos(x, y, z);
        if (!world.hasChunkAt(chunkProbe)) {
            return null;
        }
        for (int candidateY : verticalCandidates(y, verticalTolerance)) {
            BlockPos feet = new BlockPos(x, candidateY, z);
            if (isSafeFeetPosition(world, feet)) {
                return feet;
            }
        }
        return null;
    ...
    static List<Integer> verticalCandidates(int centerY, int tolerance) {
        int safeTolerance = Math.max(0, tolerance);
        List<Integer> candidates = new ArrayList<>(safeTolerance * 2 + 1);
        candidates.add(centerY);
        for (int offset = 1; offset <= safeTolerance; offset++) {
    ...
    static BlockPos findCaravanSurfaceNearY(ServerLevel world, BlockPos center, int radius,
                                             int verticalTolerance) {
        if (world == null || center == null) {
            return null;
        }
        ...
        for (int ring = 0; ring <= radius; ring++) {
            for (int dx = -ring; dx <= ring; dx++) {
                for (int dz = -ring; dz <= ring; dz++) {
                    ...
                    BlockPos candidate = safeSurfaceNearY(world, center.getX() + dx, center.getY(),
                            center.getZ() + dz, verticalTolerance);
                    if (candidate == null || !isStableCaravanSurfaceNearY(world, candidate)) {
                        continue;
                    }
                    if (isRoadBlock(world.getBlockState(candidate.below()))) {
                        return candidate;
    ...
    static boolean isStableCaravanSurfaceNearY(ServerLevel world, BlockPos center) {
        if (center == null) {
            return false;
        }
        int stableNeighbors = 0;
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                BlockPos neighbor = safeSurfaceNearY(world, center.getX() + dx, center.getY(),
                        center.getZ() + dz, 1);
                if (neighbor != null && Math.abs(neighbor.getY() - center.getY()) <= 1) {
                    stableNeighbors++;
                }
    ...
    static BlockPos findNearbySafeSurfaceNearY(ServerLevel world, int x, int y, int z,
                                                int radius, int verticalTolerance) {
        BlockPos center = safeSurfaceNearY(world, x, y, z, verticalTolerance);
        if (center != null) {
            return center;
        }
        for (int ring = 1; ring <= radius; ring++) {
            for (int dx = -ring; dx <= ring; dx++) {
                for (int dz = -ring; dz <= ring; dz++) {
                    ...
                    BlockPos candidate = safeSurfaceNearY(world, x + dx, y, z + dz, verticalTolerance);
                    if (candidate != null) {
                        return candidate;
                    }
                }
